...
//CAMERA_FRAME：系统的X轴是向右的，Y轴是向下的，Z轴是向前的。
...
main(int argc, char** argv)
{
	...
	if (1)
	{
		...
		scene_sensor_pose = Eigen::Affine3f(Eigen::Translation3f(point_cloud.sensor_origin_[0],
			...
			point_cloud.sensor_origin_[2])) *//平移
			Eigen::Affine3f(point_cloud.sensor_orientation_);  //旋转矩阵
	...
	range_image.integrateFarRanges(far_ranges);//将已有的远距离测量结果集成到深度图中
	...
	while (!viewer.wasStopped())
	{
		...
		if (1)
		{
		   ...
		   // live update - update the range image according to the selected view in the 3D viewer.
			...
			range_image.createFromPointCloud(point_cloud, angular_resolution, angular_resolution,
