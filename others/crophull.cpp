...
//1.由一片散乱的点云计算凸包形状，得到pcl::Vertices类型向量，和描述凸包形状的点云
//2.创建CropHull对象，输入1.中得到的参数，进行滤波

//https://blog.csdn.net/shenziheng1/article/details/53195489
//  https://blog.csdn.net/jiaojialulu/article/details/69352738
//// 初始化相机参数
...
int main(int argc, char** argv)
{
	...
	hull.setInputCloud(boundingbox_ptr);//一片散乱的点云
	hull.setDimension(2);//设置凸包维度
	std::vector<pcl::Vertices> polygons;//设置pcl::Vertices类型向量，保存凸包顶点
	...
	pcl::io::savePCDFile("surface_hull.pcd", *surface_hull);
	...
	for_visualizer_v->addCoordinateSystem(0.1, "cloud", v1);
	...
	for_visualizer_v->addCoordinateSystem(0.1, "surface_hull", v2);
	...
	for_visualizer_v->addCoordinateSystem(0.1, "objects", v3);
