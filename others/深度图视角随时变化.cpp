...
{
	Eigen::Vector3f pos_vector = viewer_pose * Eigen::Vector3f(0, 0, 0);//Vector3f(0, 0, 0，1)--原点坐标；pos_vector--相机坐标；T*（0，0，0）
...
main(int argc, char** argv)
{
	...
	//给传感器的位姿赋值  就是获取点云的传感器的的平移*旋转的向量
	scene_sensor_pose = Eigen::Affine3f(Eigen::Translation3f(point_cloud.sensor_origin_[0],//vector4f 传感器采集姿势（原点/平移）
		point_cloud.sensor_origin_[1],
		point_cloud.sensor_origin_[2])) *
		Eigen::Affine3f(point_cloud.sensor_orientation_);//旋转四元数
