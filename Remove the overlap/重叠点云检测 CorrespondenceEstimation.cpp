...
int main()
{
	...
	//读取PCD点云文件
	pcl::io::loadPCDFile<pcl::PointXYZ>("turn_pointcloud270s_smoothed.pcd", *cloudA);
	pcl::io::loadPCDFile<pcl::PointXYZ>("turn_pointcloud315js_smoothed.pcd", *cloudB);
	...
	core.determineReciprocalCorrespondences(*cor, 0.1);   //点之间的最大距离,cor对应索引
	...
	{
		cloudAinliers->indices.push_back(cor->at(i).index_match);
	}
